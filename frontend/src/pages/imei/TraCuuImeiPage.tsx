...
import { Button, Card, Col, Descriptions, Input, Row, Space, Tag, Timeline, Typography } from 'antd'
import {
  BarcodeOutlined,
  InboxOutlined,
    ...
    <div>
      ...
      <Card style={{ borderRadius: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.07)', marginBottom: 24 }}>
        <div style={{ display: 'flex', gap: 12, maxWidth: 600 }}>
          <Input
            size='large'
            prefix={<BarcodeOutlined style={{ color: '#ccc' }} />}
      ...
      {submitted && isError && (
        <Card style={{ borderRadius: 12, textAlign: 'center', padding: 40 }}>
          ...
          <Title level={5}>
            <Trans i18nKey='notFound.title' ns='imei' values={{ imei: submitted }} components={[<code key='0' />]} />
          </Title>
            ...
            <Card
              style={{
                ...
                background: statusColor
                  ? `${statusColor === 'green' ? '#f6ffed' : statusColor === 'blue' ? '#e6f4ff' : '#fff7e6'}`
                  : '#fafafa'
              }}
            >
              <Space size={24} wrap>
                <div>
                  ...
                  <div style={{ fontFamily: 'monospace', fontSize: 18, fontWeight: 700 }}>
                    {result.itemInfo?.imei ?? submitted}
                  </div>
                </div>
                {statusColor && (
                  <div>
                    ...
                    <div>
                      <Tag color={statusColor} style={{ fontSize: 14, padding: '2px 12px' }}>
                        {statusLabel}
                      </Tag>
                    </div>
                  </div>
            ...
            >
              {result.productInfo ? (
                <Descriptions column={1} size='small'>
                  ...
                  {/**/}
                  <Descriptions.Item label='Vị trí lưu kho'>
                    {result.locationInfo?.location ? (
                      <Space size={[0, 4]} wrap>
                        <Tag color='blue'>Khu {result.locationInfo.location.zoneName}</Tag>
                        <Tag color='blue'>Dãy {result.locationInfo.location.rowNum}</Tag>
                        <Tag color='blue'>Kệ {result.locationInfo.location.shelfNum}</Tag>
                        <Tag color='blue'>Ngăn {result.locationInfo.location.binNum}</Tag>
                      </Space>
                    ) : (
                  ...
                  {/**/}
                  <Descriptions.Item label={t('product.warrantyPeriod')}>
                    {result.productInfo.warrantyPeriod ?? '—'} {t('common:common.months')}
                  </Descriptions.Item>
              ...
              {result.saleInfo && Object.keys(result.saleInfo).length > 0 ? (
                <Descriptions column={1} size='small'>
                  {result.saleInfo.customer && (
                    <>
                      <Descriptions.Item label={t('sale.customer')}>
                        {result.saleInfo.customer.fullName}
                      </Descriptions.Item>
                      <Descriptions.Item label={t('sale.phone')}>
                        {result.saleInfo.customer.phoneNumber}
                      </Descriptions.Item>
                    </>
                  )}
                  <Descriptions.Item label={t('sale.saleDate')}>{date(result.saleInfo.saleDate)}</Descriptions.Item>
                  ...
                  <Descriptions.Item label={t('sale.paymentMethod')}>
                    {t(`common:status.payment.${result.saleInfo.paymentMethod}`, {
                      defaultValue: result.saleInfo.paymentMethod
                    })}
                  </Descriptions.Item>
                </Descriptions>
              ) : (
