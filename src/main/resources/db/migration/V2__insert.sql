...
MERGE INTO users t
    ...
    WHEN NOT MATCHED THEN
        INSERT (username, full_name, password, role_id, status, is_password_reset, is_deleted)
            VALUES (s.uname, s.fname, s.pwd, (SELECT id FROM roles WHERE name = s.rname), 'ACTIVE', 0, 0);
